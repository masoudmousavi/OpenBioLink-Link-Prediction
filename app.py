...
#     fully_connected_for_head_dims=[32, 16], 
#     fully_connected_for_tail_dims=[32, 16],
#     dropout={
#         "emb": 0.225,
#         "conv": 0.15,
#         "fc": 0.375
...
model = torch.load('model50000-16.pth').to(Global.DEVICE.value)
...
plus_model = PlusGNN(
  x_feature='one-hot',
  conv_dims=[8, 8, 8],
  embedding_dims=[features.max() + 1, 64],
  fully_connected_for_head_dims=[8],
  fully_connected_for_tail_dims=[8],
  dropout={
        "emb": 0.225,
        "conv": 0.15,
        "fc": 0.375
...
print('*' * 50)
# print(Global)
margin_ranking_loss_fn = torch.nn.MarginRankingLoss(
  margin=Global.MARGIN.value, reduction=Global.REDUCTION.value
)
opt = Adam(plus_model.parameters()) 
...
try:
  ...
  ############################## TRAIN Graph Maker #####################################
    for i in tqdm(range(iteration * Global.MINI_BATCH_SIZE.value, train_set.shape[0] - Global.MINI_BATCH_SIZE.value, Global.MINI_BATCH_SIZE.value)):
      ...
      loss, positive_score, negative_score = train(plus_model, train_graph, opt, margin_ranking_loss_fn)
      epoch_loss += loss
    ...
    if epoch % 5 == 0 and epoch:
      evaluate_hits_at_10(plus_model, mode='validation')
...
######################################## Test ########################################
try:
  evaluate_hits_at_10(plus_model, 'testing')
except :
  pass